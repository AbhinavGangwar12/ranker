...
const CATEGORIES = {
    general: { fetcher: fetchReasoningModels, title: "Global Leaderboard (MMLU-Pro)", subtitle: "Comprehensive benchmark analysis across top 10 models.", footer: "Showing top 10 models based on TIGER-Lab MMLU-Pro benchmark." },
    mathematics: { fetcher: fetchMathsModels, title: "Mathematics Leaderboard (GSM8K)", subtitle: "Top 10 models ranked by grade-school math benchmark performance.", footer: "Showing top 10 models based on GSM8K benchmark." },
    code: { fetcher: fetchCodeModels, title: "Code Leaderboard (SWE-Bench)", subtitle: "Top 10 models ranked by human evaluation of code generation performance.", footer: "Showing top 10 models based on SWE-Bench benchmark." },
    "instruction-following": { fetcher: fetchInstructionModels, title: "Instruction Following Leaderboard (IFEval)", subtitle: "Top 10 models ranked by performance on instruction following tasks.", footer: "Showing top 10 models based on IFEval benchmark." },
    "multi-tasking": { fetcher: fetchMultiModels, title: "Multi-Tasking Leaderboard (MMMU)", subtitle: "Top 10 models ranked by performance on multi-tasking benchmarks.", footer: "Showing top 10 models based on MMMU benchmark." },
    "long-context": { fetcher: fetchLongModels, title: "Long Context Leaderboard (LongBench)", subtitle: "Top 10 models ranked by performance on long-context benchmarks.", footer: "Showing top 10 models based on LongBench benchmark." }
};

let currentCategory = null;
let currentThreadId = null;

const RANK_GLOW = ["text-cyan-300 border-cyan-400", "text-violet-300 border-violet-400", "text-fuchsia-300 border-fuchsia-400"];

function downloadCSV(models, category) {
    const headers = ['Rank', 'Name', 'Source', 'Score'];
    const rows = models.map(m => [m.Rank, m.Name, m.DataSource, m.Score]);
    const escapeCell = (val) => { const str = String(val); return /[",\n]/.test(str) ? `"${str.replace(/"/g, '""')}"` : str; };
    const csv = [headers, ...rows].map(row => row.map(escapeCell).join(',')).join('\n');
    const blob = new Blob([csv], { type: 'text/csv;charset=utf-8;' });
    const url = URL.createObjectURL(blob);
    const a = document.createElement('a');
    a.href = url; a.download = `${category}-leaderboard.csv`;
...
function renderTable(models, meta, category) {
    if (!models || models.length === 0) {
        contentDiv.innerHTML = `<div class="p-6 text-center text-slate-500 font-mono text-sm">// no data returned</div>`;
        return;
    }
    contentDiv.innerHTML = `
        <div class="mb-6 flex flex-col md:flex-row md:items-end justify-between animate-slide-up gap-4">
            <div>
                <h1 class="text-lg font-semibold mb-1 text-slate-100">${meta.title}</h1>
                <p class="text-xs text-slate-500 font-mono">${meta.subtitle}</p>
            </div>
            <button id="export-csv-btn" class="flex items-center gap-2 px-4 py-2 text-sm bg-cyan-500 text-slate-950 font-semibold hover:bg-cyan-400 transition-colors">
                <i class="fa-solid fa-download"></i>Export CSV
            </button>
        </div>
        <div class="flex flex-col gap-2 animate-fade-in">
            ${models.map(model => `
                <div class="tech-card flex items-center gap-4 px-4 py-3">
                    <span class="w-7 h-7 border ${RANK_GLOW[model.Rank - 1] || 'text-slate-500 border-slate-700'} flex items-center justify-center text-xs font-mono font-semibold flex-shrink-0">${model.Rank}</span>
                    <i class="fa-solid ${model.icon} text-slate-500 w-4 text-center"></i>
                    <span class="font-medium text-slate-100 truncate min-w-0">${model.Name}</span>
                    ${model.Rank === 1 ? `<span class="border border-cyan-400 text-cyan-300 text-[10px] px-2 py-0.5 font-mono hidden sm:inline-block">SOTA</span>` : ''}
                    <span class="hidden md:inline text-sm text-slate-500 ml-2">${model.DataSource}</span>
                    <div class="ml-auto flex items-center gap-3">
                        <div class="w-20 h-1.5 bg-slate-800 overflow-hidden hidden sm:block">
                            <div class="h-full bg-gradient-to-r from-cyan-500 to-violet-500 transition-all duration-1000 ease-out" style="width: 0%" data-width="${model.scoreNum}%"></div>
                        </div>
                        <span class="font-mono font-medium text-cyan-300 tabular-nums w-16 text-right">${model.Score}</span>
                    </div>
                </div>
            `).join('')}
        </div>
        <div class="text-center mt-6 text-sm text-slate-600 font-mono">${meta.footer}</div>
    `;
    document.getElementById('export-csv-btn').addEventListener('click', () => downloadCSV(models, category));
    setTimeout(() => document.querySelectorAll('[data-width]').forEach(el => el.style.width = el.getAttribute('data-width')), 50);
}
...
async function loadCategory(category) {
    if (category === 'chat') {
        currentCategory = category; currentThreadId = null;
        setActiveNav(category); renderChatWindow();
        return;
    }
    const meta = CATEGORIES[category];
    if (!meta) return;
    currentCategory = category;
    setActiveNav(category);
    contentDiv.innerHTML = `<div class="p-6 text-center text-slate-500 font-mono text-sm">loading rankings...</div>`;
    const models = await meta.fetcher();
    ...
    contentDiv.innerHTML = `
        <div class="max-w-sm mx-auto mt-12 animate-slide-up">
            <div class="tech-card p-6">
                <h1 class="text-lg font-semibold text-slate-100 mb-1 font-mono">${mode === 'login' ? 'LOG_IN' : 'CREATE_ACCOUNT'}</h1>
                <p class="text-xs text-slate-500 mb-5">${mode === 'login' ? 'Log in to use the Chat with us feature.' : 'Sign up to get started.'}</p>
                <div id="auth-error" class="hidden text-xs text-fuchsia-400 border border-fuchsia-500/40 bg-fuchsia-500/10 px-3 py-2 mb-4"></div>
                <form id="auth-form" class="flex flex-col gap-3">
                    <input type="email" id="auth-email" required placeholder="Email" autocomplete="email"
                        class="bg-slate-900 border border-slate-700 px-3 py-2 text-sm outline-none focus:border-cyan-400 text-slate-100" />
                    <input type="password" id="auth-password" required minlength="6" placeholder="Password" autocomplete="current-password"
                        class="bg-slate-900 border border-slate-700 px-3 py-2 text-sm outline-none focus:border-cyan-400 text-slate-100" />
                    <button type="submit" id="auth-submit-btn" class="bg-cyan-500 text-slate-950 py-2 text-sm font-semibold hover:bg-cyan-400 transition-colors">
                        ${mode === 'login' ? 'Log in' : 'Sign up'}
                    </button>
                </form>
                <p class="text-xs text-slate-500 mt-4 text-center">
                    ${mode === 'login' ? "Don't have an account?" : "Already have an account?"}
                    <button id="auth-toggle-btn" class="text-cyan-400 font-medium hover:underline">${mode === 'login' ? 'Sign up' : 'Log in'}</button>
                </p>
            </div>
        </div>
    `;
    document.getElementById('auth-toggle-btn').addEventListener('click', () => renderAuthView(mode === 'login' ? 'register' : 'login'));
    document.getElementById('auth-form').addEventListener('submit', async (e) => {
        e.preventDefault();
        const email = document.getElementById('auth-email').value.trim();
        const password = document.getElementById('auth-password').value;
        ...
        const btn = document.getElementById('auth-submit-btn');
        errBox.classList.add('hidden');
        btn.disabled = true; btn.textContent = mode === 'login' ? 'Logging in...' : 'Signing up...';
        try {
...
// Chat result: HUD-style ranked bars instead of bubbles — sharper, reads as a data readout
function renderResultBox({ verdict, top3 } = {}) {
    const resultDiv = document.getElementById('chat-result');
    if (!resultDiv) return;
    if (!verdict) { resultDiv.innerHTML = ''; return; }

    const sorted = [...(top3 || [])].sort((a, b) => b.confidence - a.confidence);
    const barColors = ['from-cyan-500 to-cyan-300', 'from-violet-500 to-violet-300', 'from-fuchsia-500 to-fuchsia-300'];

    const rows = sorted.map((t, i) => `
        <div class="border border-slate-800 bg-slate-900/60 p-3">
            <div class="flex items-center justify-between mb-1.5">
                <span class="font-mono text-sm text-slate-100 uppercase tracking-wide">${String(i + 1).padStart(2, '0')} · ${t.type.replace(/-/g, ' ')}</span>
                <span class="font-mono text-xs text-cyan-300">${t.confidence.toFixed(1)}/10</span>
            </div>
            <div class="w-full h-1.5 bg-slate-800 mb-2 overflow-hidden">
                <div class="h-full bg-gradient-to-r ${barColors[i] || barColors[2]}" style="width:${t.confidence * 10}%"></div>
            </div>
            <p class="text-xs text-slate-400 leading-relaxed">${t.description}</p>
        </div>
    `).join('');
    ...
    resultDiv.innerHTML = `
        <div class="p-4 flex flex-col gap-2">
            ${rows}
            <div class="border border-cyan-500/30 bg-cyan-500/5 p-4 mt-2">
                <label class="text-[10px] font-mono uppercase tracking-widest text-cyan-400 block mb-2">Verdict</label>
                <p class="text-sm text-slate-200 leading-relaxed">${verdict}</p>
            </div>
        </div>
    `;
}
...
async function loadHistory() {
    const listDiv = document.getElementById('chat-history-list');
    if (!listDiv) return;
    listDiv.innerHTML = `<div class="text-xs text-slate-600 font-mono px-1">loading history...</div>`;
    try {
        const threads = await listThreads();
        if (!threads || threads.length === 0) {
            listDiv.innerHTML = `<div class="text-xs text-slate-600 font-mono px-1">no chats yet — send a message to start one.</div>`;
            return;
        }
        listDiv.innerHTML = threads.map(t => `
            <div class="tech-card flex items-center justify-between px-4 py-3 cursor-pointer history-item" data-thread-id="${t.id}">
                <div class="min-w-0">
                    <div class="text-sm text-slate-100 truncate">${t.title || 'Untitled chat'}</div>
                    <div class="text-xs text-slate-500 font-mono mt-0.5">${formatDate(t.updated_at)}</div>
                </div>
                <button class="delete-thread-btn text-slate-500 hover:text-fuchsia-400 flex-shrink-0 ml-3 px-2 py-1" data-thread-id="${t.id}" title="Delete this chat">
                    <i class="fa-solid fa-trash"></i>
                </button>
            </div>
        `).join('');
        listDiv.querySelectorAll('.history-item').forEach(item => item.addEventListener('click', () => openThread(item.dataset.threadId)));
        listDiv.querySelectorAll('.delete-thread-btn').forEach(btn => {
            btn.addEventListener('click', async (e) => {
                e.stopPropagation();
                if (!confirm('Delete this chat? This cannot be undone.')) return;
        ...
        listDiv.innerHTML = `<div class="text-xs text-fuchsia-400 px-1">${err.message}</div>`;
    }
}
...
async function sendMessage() {
    ...
    try {
        ...
        renderResultBox({ verdict: `Something went wrong: ${err.message}`, top3: [] });
    } finally {
        sendBtn.disabled = false;
        sendBtn.innerHTML = `Send <i class="fa-solid fa-paper-plane"></i>`;
    }
...
function renderChatWindow() {
    contentDiv.innerHTML = `
        <div class="mb-6 animate-slide-up">
            <h1 class="text-lg font-semibold mb-1 text-slate-100 font-mono">CHAT_WITH_US</h1>
            <p class="text-xs text-slate-500">Enter your prompt on the left — we'll suggest which category fits it best.</p>
        </div>
        <div class="flex flex-col md:flex-row gap-4 min-h-[400px] animate-fade-in">
            <div class="flex-1 tech-card p-4 flex flex-col">
                <label class="text-[10px] font-mono uppercase tracking-widest text-slate-500 mb-2">Your input</label>
                <textarea id="chat-input" class="flex-1 w-full resize-none bg-slate-950 p-3 text-sm text-slate-100 outline-none border border-slate-800 focus:border-cyan-400" placeholder="Type something..."></textarea>
                <div class="flex items-center justify-between mt-3">
                    <button id="chat-new-btn" class="text-xs text-slate-500 hover:text-slate-200 transition-colors font-mono">+ new_chat</button>
                    <button id="chat-send-btn" class="flex items-center gap-2 px-4 py-2 bg-cyan-500 text-slate-950 font-semibold hover:bg-cyan-400 transition-colors text-sm">
                        Send <i class="fa-solid fa-paper-plane"></i>
                    </button>
                </div>
            </div>
            <div class="flex-1 tech-card flex flex-col overflow-hidden">
                <label class="text-[10px] font-mono uppercase tracking-widest text-slate-500 px-4 pt-4">Result</label>
                <div id="chat-result" class="flex-1 w-full overflow-y-auto"></div>
            </div>
        </div>
        <div class="mt-8 animate-fade-in">
            <h2 class="text-sm font-medium text-slate-300 mb-3 font-mono">RECENT_CHATS</h2>
            <div id="chat-history-list" class="flex flex-col gap-2"></div>
        </div>
    `;
    document.getElementById('chat-send-btn').addEventListener('click', sendMessage);
...
document.querySelectorAll('.nav-btn[data-category]').forEach(btn => {
    btn.addEventListener('click', () => {
        const category = btn.dataset.category;
        if (category === 'chat' && !isLoggedIn()) { renderAuthView('login'); return; }
        loadCategory(category);
    });
});
...
// Ad banner dismiss — remembered across visits
const banner = document.getElementById('ad-banner');
if (localStorage.getItem('ad_banner_dismissed') === '1' && banner) banner.remove();
document.getElementById('ad-banner-close')?.addEventListener('click', () => {
    localStorage.setItem('ad_banner_dismissed', '1');
    banner?.remove();
});
...
async function init() {
    setHeaderGreeting();
    if (isLoggedIn()) await fetchCurrentUser();
    renderAuthButton();
    loadCategory('general');
}
